...
class orangex(ImplicitAPI, Exchange):
    ...
    def fetch_tickers(self, symbols=None, params={}):
        ...
        for t in types:
            ...
            response = self.public_get_tickers(self.extend({'currency': currency}, params))
    ...
    def sign_in(self, params={}):
        ...
        response = self.public_post_auth(rpc_request)
        result = self.safe_value(response, 'result', {})
    ...
    def fetch_balance(self, params={}):
        ...
        response = self.private_post_get_assets_info(self.extend(request, params))
        result = self.safe_value(response, 'result', {})
    ...
    def create_order(self, symbol: str, type: str, side: str, amount: float, price=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        side = side.lower()
    ...
    def cancel_all_orders(self, symbol: str = None, params={}):
        ...
        if symbol is not None:
            ...
            return self.private_post_cancel_all_by_instrument(self.extend(request, params))
        ...
        params = self.omit(params, 'currency')
        request['currency'] = currency
        return self.private_post_cancel_all_by_currency(self.extend(request, params))

    def fetch_order(self, id: str, symbol: str = None, params={}):
        request = {
            'order_id': id,
        }
        response = self.private_post_get_order_state(self.extend(request, params))
        result = self.safe_value(response, 'result', {})
        market = None
        if symbol is not None:
            self.load_markets()
    ...
    def fetch_open_orders(self, symbol: str = None, since=None, limit=None, params={}):
        self.load_markets()
        request = {}
        market = None
        if symbol is not None:
            market = self.market(symbol)
            request['instrument_name'] = market['id']
            response = self.private_post_get_open_orders_by_instrument(self.extend(request, params))
        else:
            ...
            params = self.omit(params, 'currency')
            request['currency'] = currency
            response = self.private_post_get_open_orders_by_currency(self.extend(request, params))
        result = self.safe_list(response, 'result', [])
        return self.parse_orders(result, market, since, limit)

    def fetch_orders(self, symbol: str = None, since=None, limit=None, params={}):
        self.load_markets()
        request = {}
        market = None
        if limit is not None:
        ...
        if symbol is not None:
            market = self.market(symbol)
            request['instrument_name'] = market['id']
            response = self.private_post_get_order_history_by_instrument(self.extend(request, params))
        else:
            ...
            params = self.omit(params, 'currency')
            request['currency'] = currency
            response = self.private_post_get_order_history_by_currency(self.extend(request, params))
        result = self.safe_list(response, 'result', [])
        return self.parse_orders(result, market, since, limit)

    def fetch_my_trades(self, symbol: str = None, since=None, limit=None, params={}):
        self.load_markets()
        request = {}
        market = None
        ...
        if order_id is not None:
            ...
            response = self.private_post_get_user_trades_by_order(self.extend(request, params))
        elif symbol is not None:
            market = self.market(symbol)
            request['instrument_name'] = market['id']
            response = self.private_post_get_user_trades_by_instrument(self.extend(request, params))
        else:
            ...
            params = self.omit(params, 'currency')
            request['currency'] = currency
            response = self.private_post_get_user_trades_by_currency(self.extend(request, params))
        result = self.safe_value(response, 'result', {})
    ...
    def fetch_positions(self, symbols=None, params={}):
        ...
        response = self.private_post_get_positions(self.extend(request, params))
        result = self.safe_list(response, 'result', [])
        positions = []
    ...
    def close_position(self, symbol: str, side: str = None, params={}):
        self.load_markets()
        market = self.market(symbol)
        request = {
            'instrument_name': market['id'],
        ...
        response = self.private_post_close_position(self.extend(request, params))
        result = self.safe_value(response, 'result', {})
        order = self.safe_value(result, 'order', {})
        order_id = self.safe_string(order, 'order_id')
        order = self.extend(order, request)
